...
function IsSuspended() {
  ...
  const { suspensionDuration, suspensionEndDate, suspensionReason } =
    useSelectorList();
  const closeModal = () => {
    dispatch(handleModal({ field: 'isSuspended', value: false }));
  };
  return (
    <ModalContainer>
      <ModalBox>
        ...
        <h4>계정 정지 안내</h4>
        <h6>현재 귀하의 계정은 {suspensionDuration} 동안 정지되었습니다.</h6>
        <h6>정지 사유: {suspensionReason}</h6>
        <h6>정지 해제 예정일: {formatDateToSuspend(suspensionEndDate)}</h6>
